...
with DAG('multi_cities_dag',
    ...
    for city in cities:
        # Each city gets its own task
        task_id = f'is_weather_api_ready_multi_{city.replace(" ", "_").lower()}'
        task_id = f'extract_weather_data_multi_{city.replace(" ", "_").lower()}'
        task_id = f'transform_load_weather_data_multi_{city.replace(" ", "_").lower()}'

        #Check (poke) the endpoint to see it is working
        is_weather_api_ready_multi = HttpSensor(
            task_id='is_weather_api_ready_multi',
        ...
        # Here you can add downstream tasks for each city as needed
        transform_load_weather_data_multi = PythonOperator(
        task_id = 'transform_load_weather_data_multi',
